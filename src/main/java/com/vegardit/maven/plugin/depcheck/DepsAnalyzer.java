...
public class DepsAnalyzer {
   ...
   public ScanResult scan(final boolean checkForUnusedDependencies, final boolean checkForUsedTransitiveDependencies) throws MojoExecutionException {
      ...
      log.info(" => References to " + referencedClasses.size() + " external classes found.");

      /*
       * early exit if no references to external classes were found
       */
      ...
      for (final Artifact directDep : MavenUtils.withoutRuntimeAndTestScoped(mojo.getDirectDependencies())) {
      ...
      if (checkForUnusedDependencies) {
         if (result.hasUnusedDirectDependencies()) {
            log.warn(" => " + result.unusedDirectDependencies.size() + " potentially unused dependencies found:");
      ...
      if (checkForUsedTransitiveDependencies) {
         ...
         if (result.usedClassesOfTransitiveDependencies.isEmpty()) {
            ...
            log.info(" => References to " + result.usedClassesOfTransitiveDependencies.size() + " classes of " + result.getUsedTransitiveDependencies().size()
   ...
   private MavenProject getReactorProject(final Artifact artifact) throws NoSuchElementException {
      return mojo.getReactorProjects().stream().filter(p -> //
      p.getGroupId().equals(artifact.getGroupId()) && //
         p.getArtifactId().equals(artifact.getArtifactId()) && //
         p.getVersion().equals(artifact.getVersion()) //
      ).findFirst().get();
   ...
   private Set<String> scanArtifactForDeclaredClasses(final Artifact artifactWithJar) throws IOException {
      ...
      try (JarFile jar = new JarFile(artifactWithJar.getFile())) {
         ...
         if (isVerbose) {
            log.info(" => Found " + classesDeclaredInJar.size() + " classes in inspected JAR file");
         }
   ...
   private Tuple2<Set<String>, Set<String>> scanDirectoryForDeclaredAndReferencedClasses(final Path classDirectory) throws IOException {
      ...
      new DirectoryWalker<Object>() {
         {
            walk(classDirectory.toFile(), null);
         }

         // analyse the found class files
         @Override
         protected void handleFile(final File file, final int depth, final Collection<Object> results) throws IOException {
            ...
            try (InputStream classByteCode = new BufferedInputStream(new FileInputStream(file))) {
